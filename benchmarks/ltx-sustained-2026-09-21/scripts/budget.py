...
def compute_gate(state, entry_id):
    """Use a recorded single-run allocation of the unchanged total budget."""
    exception = state.get("compute_gate_exceptions", {}).get(entry_id)
    if exception is None:
        return NEW_COMPUTE_CENTS
    limit = exception.get("limit_cents")
    if (type(limit) is not int or not NEW_COMPUTE_CENTS <= limit < CAP_CENTS
            or not exception.get("reason") or not exception.get("authorization_basis")):
        raise ValueError("Invalid scoped compute allocation")
    return limit
...
def reserve(state, entry_id, amount, phase):
    ...
    if phase != "closeout" and current + quote > compute_gate(state, entry_id):
        raise ValueError("new compute would consume the closeout reserve")
    if current + quote > CAP_CENTS:
        raise ValueError(f"total exposure would exceed USD {CAP_CENTS / 100:.2f}")
...
def summary(state):
    ...
    return {"cap_usd": f"{CAP_CENTS / 100:.2f}", "exposure_usd": f"{total / 100:.2f}",
