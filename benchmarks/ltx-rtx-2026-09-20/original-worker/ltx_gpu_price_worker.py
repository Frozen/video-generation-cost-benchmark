"""One paired five-second queue with two full warmups; no provisioning.
...
from load_window import (Journal, read_manifest, run_queue, sha256, validate_manifest,
...
def validate_profile(plan):
    ...
    if plan["profile"] != profile(plan["profile"]["gpu_name"]):
        raise ValueError("Only the frozen resident five-second native-audio profile is supported")
    if len(plan["requests"]) != 10 or plan["limits"]["max_requests"] != 10:
        raise ValueError("Ten paired requests are required")
    seeds = [r["seed"] for r in plan["requests"]] + plan["technical_warmup_seeds"]
    if len(seeds) != 12 or len(set(seeds)) != 12 or any(type(s) is not int or not 0 <= s < 2**32 for s in seeds):
        raise ValueError("Ten measured and two separate warmup seeds are required")
...
def require_complete_queue(result):
    if result["submitted"] != 10 or result["not_submitted"] != 0 or result["stop_reason"] != "request_limit":
        raise RuntimeError("Incomplete ten-request comparison; retain artifacts and report the failure")
...
def serve(manifest, base):
    ...
    try:
        ...
        if time.time() + required_suite_seconds(plan) >= plan["limits"]["shutdown_deadline_epoch"]:
            raise TimeoutError("Insufficient time for both full warmups, all ten requests and export")
        ...
        if time.time() + 10 * plan["limits"]["request_bound_seconds"] + plan["limits"]["export_reserve_seconds"] >= plan["limits"]["shutdown_deadline_epoch"]:
