...
September 17, 2026. **No self-host Pod created or inference submitted.**
The three [fal samples](VARIANT_RESULTS.md) are complete; do not rerun them.
This is a readiness record, not a completed launch plan.
...
operator's applicable location and the deployment region remain unconfirmed.
...
- Selected candidate: 1 x B300, 288 GB, Secure Cloud, SGLang Diffusion, H3 Base
  FL2VA. Latest catalog observation: USD 7.89/hour, LOW availability in EU-NL-1,
  EUR-IS-1 and US-WA-2. No capacity is reserved; no all-in quote is established.
...
## Rental deadline still unresolved
...
No independent shutdown guard is installed or verified. Runtime/container pins,
host/storage sizing, SSH, export and the full rental reservation also remain open.
Do not create a paid Pod until these gates are resolved. Do not silently replace
H3 Base with fal Max/Turbo, change the GPU plan or submit extra API requests.
