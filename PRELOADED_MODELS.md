# Prepare persistent model storage before the next GPU rental

Status, September 18, 2026: **requested, not provisioned or uploaded**. No GPU
Pod or network volume remains from the failed acceleration trial. The operator
requested that weights be staged first and mounted when the next test is ready.
...
## Current blockers and catalog evidence
...
- A live catalog read listed **AP-JP-1, EU-NL-1 and EUR-IS-3** with STANDARD
  ...
  cards can be allocated together; no new allocation was attempted.
...
- EU-NL-1 reports STANDARD storage, H100 SXM stock `LOW`, and general-purpose CPU
  ...
  in the target H100 region without S3. It has not been provisioned or timed.
- The account's network-volume list was empty. The proposed CPU downloader and
  48-hour disk-retention window have been submitted to the operator for
  confirmation. GPU allocation is not part of that request.

## Proposed storage, not an existing charge
...
estimate/proposal, not a storage purchase or completed upload.
...
problems and both require verification before another paid GPU trial.
